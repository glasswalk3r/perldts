package DTS_UT::Test::Harness::Straps::Eval;

=pod

=head1 NAME

DTS_UT::Test::Harness::Straps::Eval - subclass of C<Test::Harness::Straps> to eval test file code instead of forking 
a process to execute it.

=head1 DESCRIPTION

C<DTS_UT::Test::Harness::Straps::Eval> will execute a test file code (Perl code, of course) by using an C<eval> instead
...
Expects two parameters (in this order): an scalar reference with the Perl code to C<eval> and a complete pathname of
...
sub analyze_file {
    ...
    # executes the text script passed as a reference
    eval($perl_code);
