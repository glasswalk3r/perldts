...
The C<DTS> class does not much: it will server only as an interface class, since it cannot be instancied or the
...
C<DTS> distribution will be moved to a new registered namespace: C<Win32::SqlServer::DTS>, so release 0.07 will be the last
...
that inherints from C<DTS> (almost of all classes in the API).
