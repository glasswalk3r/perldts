...
Instantiates a new C<DTS::Assigment> object. Expects as parameter a C<DynamicPropertiesTaskAssignment> object. 
Unless you want to extend the C<DTS::Assignment> class, you will want to fetch C<DTS::Assignment> objects using 
the C<get_properties> method from L<DTS::Task::DynamicProperty|DTS::Task::DynamicProperty> class.
...
sub new {
    ...
    # DTS::Assignment::Destination object
    $self->{destination}
...
This method should be override by any subclass of C<DTS::Assignment>. If invoked but not overrided, it will abort
...
Returns a C<DTS::Assignment::Destination> object. See L<DTS::Assignment::Destination> for more details about how
to use Destination objects.

A C<DTS::Assignment::Destination> object is not part of the official MS SQL Server DTS API, but is easier to use and
...
invoke this method, the C<DTS::Assignment> should not have invoke the C<kill_sibling> method before, since writing
...
Once the string is modified successfully in the package, the C<DTS::Assignment::Destination> will be modified as well
...
=item *
L<DTS::Assignment::Destination> and all subclasses of it at C<perldoc>.
